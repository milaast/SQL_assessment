...
SELECT m.name AS model_name, b.name AS brand_name, b.headquarters FROM brands AS b JOIN models AS m ON b.brand_id = m.brand_id WHERE b.name = 'Ford' AND m.name = 'Mustang';
...
SELECT m.year, m.name, m.model_id, a.name AS awards_name FROM models AS m LEFT JOIN awards AS a ON (a.winner_id = m.model_id) WHERE m.year >= 1964;
...
SELECT b.name, b.founded FROM brands AS b LEFT JOIN models AS m USING (brand_id) WHERE m.brand_id is NULL;
...
SELECT b.brand_id FROM brands AS b JOIN models AS m ON m.brand_id = b.brand_id GROUP BY b.brand_id HAVING COUNT(*) > 5;
